...
export function PostRegisterPage() {
  ...
  return (
    <div className="min-h-screen bg-background pt-20 pb-12">
      ...
      {loading ? (
        <LoadingScreen variant="blue" fullScreen={false} />
      ) : error || !registration ? (
        <div className="max-w-md mx-auto px-4 mt-8">
          <PageCard className="text-center max-w-sm mx-auto flex flex-col items-center gap-4 p-6 shadow-xl border-border bg-white/95 backdrop-blur-md rounded-2xl">
            <AlertCircle className="w-12 h-12 text-destructive mx-auto" />
            <h2 className="text-lg font-bold" style={{ color: NAVY }}>Registration Not Found</h2>
            <p className="text-xs text-muted-foreground leading-relaxed">
              We couldn't retrieve your registration details for reference <strong className="text-foreground">{qrRef || "N/A"}</strong>. Please check your ticket link.
            </p>
